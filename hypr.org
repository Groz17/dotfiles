...
mapfile -t songs < <(fd . "$music_dir" -tf -X file -i00 {} \; | gawk 'BEGIN{RS="\x00/";FS="\x00"}$2~/^audio/{print NR==1?$1:"/"$1}'| rofi -sort -i -normalize-match -multi-select -dmenu -p "Search Song" -display-columns 5,6 -display-column-separator /)
