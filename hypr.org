...
#+begin_src bash :tangle ~/.config/hypr/scripts/recognize_song :tangle-mode +x
#!/usr/bin/env bash
dunstify "Recognizing song..."
[[ `hostnamectl -j|jq -r .Chassis` == "Desktop" ]] && { desktop_device="-d Webcam\ C270\ Mono"; deskop_command="flatpak run com.github.marinm."; }
song="$(timeout 10 ${desktop_command:-songrec} recognize ${desktop_device:+$desktop_device} --json)"
if [ -n "$song" ]; then
...
#+begin_src bash :tangle ~/.config/hypr/scripts/songs :tangle-mode +x
#!/usr/bin/env bash
...
***** Search on YT
#+begin_src bash :tangle ~/.config/hypr/scripts/yt_songs :tangle-mode +x
#!/usr/bin/env bash
SOCKET=/tmp/mpvsocket
[ -S "$SOCKET" ] && echo '{ "command": ["quit"] }' | socat - "$SOCKET"
...
#+begin_src bash :tangle ~/.config/hypr/scripts/notifications :tangle-mode +x
#!/usr/bin/env bash
