...
* Hyprland
:PROPERTIES:
:header-args: :tangle (concat (getenv "XDG_CONFIG_HOME") "/hypr/hyprland.conf")
:END:
...
#+begin_src bash :tangle (concat (getenv "XDG_CONFIG_HOME") "/hypr/scripts/recognize_song") :noweb yes :shebang "#!/usr/bin/env bash"
...
#+begin_src bash :tangle (concat (getenv "XDG_CONFIG_HOME") "/hypr/scripts/songs") :shebang "#!/usr/bin/env bash"
...
***** Search on YT
#+begin_src bash :tangle (concat (getenv "XDG_CONFIG_HOME") "/hypr/scripts/yt_songs") :shebang "#!/usr/bin/env bash"
SOCKET=/tmp/mpvsocket
[ -S "$SOCKET" ] && echo '{ "command": ["quit"] }' | socat - "$SOCKET"
...
#+begin_src bash :tangle (concat (getenv "XDG_CONFIG_HOME") "/hypr/scripts/notifications") :shebang "#!/usr/bin/env bash"
...
bind = $mainMod , w, exec, [float] $terminal -e nvim ~/vimwiki/Personal/diary/`date +%F`.md -c 'set ft=' -c 'norm! Go' -cstartinsert -c 'norm! zt'
...
* Hyprlock
:PROPERTIES:
:header-args: :tangle (concat (getenv "XDG_CONFIG_HOME") "/hypr/hyprlock.conf")
:END:
...
* Hypridle
:PROPERTIES:
:header-args: :tangle (concat (getenv "XDG_CONFIG_HOME") "/hypr/hypridle.conf")
:END:
...
* Hyprpaper
:PROPERTIES:
:header-args: :tangle (concat (getenv "XDG_CONFIG_HOME") "/hypr/hyprpaper.conf")
:END:
...
* hyprsunset
#+begin_src hyprlang :tangle (concat (getenv "XDG_CONFIG_HOME") "/hypr/hyprsunset.conf")
max-gamma = 150
