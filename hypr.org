...
if hyprctl activewindow -j | jq -e 'select(.title | endswith (" - Nvim"))'; then
  socket=$(find /run/user/1000 -maxdepth 1 -name 'nvim.*.0' -type s -printf '%A@ %p\n' | sort -rn | head -1 | cut -d' ' -f2-)
  data="$(nvim -u NONE --headless --server "$socket" --remote-expr "getreg('\"')")"
else
  data="$(wl-paste)"
fi
printf "\n%s" "$data" >> ~/vimwiki/Personal/diary/$(date +%F).md && dunstify "Saved $data in diary"
#+end_src
...
background {
    monitor =
        ...
        # blur_passes = 2
    path = ~/dotfiles/pics/lockscreen/kanagawa.jpg
    blur_passes = 1
    contrast = 0.8916
    brightness = 0.8172
    vibrancy = 0.1696
...
preload=~/dotfiles/pics/wallpapers/kanagawa_vaporwave.jpg
wallpaper=,~/dotfiles/pics/wallpapers/kanagawa_vaporwave.jpg
# ipc = off
#+end_src
