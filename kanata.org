...
*** Ctrl
#+begin_src kbd
  ;;d (tap-hold-press-timeout 200 250 d lctl (macro S-d))
...
;; if d and m pressed at the same time-> double ctrl, for things like c-cr (c-c-m)
  ...
  ;;k (tap-hold-press-timeout 200 250 k rctl (macro S-k))
...
#+begin_src kbd
  ...
  ;;f (tap-hold-press-timeout 200 250 f lalt (macro S-f))
  ;;f (tap-hold-press-timeout 200 250 f (layer-while-held symbols) (macro S-f))
  ...
  ;;j (tap-hold-press-timeout 200 200 j j (macro S-j))
  ;;j (tap-hold-press-timeout 200 200 j alt (macro S-j))
