...
recognize (cmd bash -c "sudo -u `id -un` ~/.config/hypr/scripts/recognize_song")
fx (cmd bash -c "sudo -u `id -un` ~/.config/hypr/scripts/fx")
;; uppercase O for playlist?
loop (cmd bash -c "sudo -u `id -un` playerctl loop $( [ $(playerctl loop) = None ] && printf Track || printf None )")
