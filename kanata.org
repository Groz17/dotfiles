...
#+end_src
** One-shot (C-c ...)
x and , harder to press than c and m (one-shot bindings should be less frequent than localleader bindings)
#+begin_src elisp
...
** Localleader (C-c c-...)
or find strategy to add ending control... maybe tap dance? or if they're pressed at the same time (q->C-x C-k,q & p->C-x C-k C-)
#+begin_src kbd
...
;; bind to C-c C-, C-c C-c would be very comfortable with mc
...
;; fai che quando g e premuto, h diventa ctrl e viceversa (for emacs help mappings)
  ...
  p (tap-hold-press-timeout 175 175 p (macro S-p) (macro S-p))
  q (tap-hold-press-timeout 175 175 q (macro S-q) (macro S-q))
